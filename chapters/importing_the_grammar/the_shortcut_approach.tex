...
Or did we?
When producing code, the language really does what one would expect. However, when user starts deleting code, the layer problem occurs once more.
\\
...
Imagine, that some alternative of some other rule contains reference to this altered \parserrule{content} rule (regardless of its quantitative operator).
...
But each alternative of the altered \parserrule{content} rule requires different number of children to be inserted.
...
A small example might demonstrate this better. We have an \interface{IContent} [1..1] child. If we disregarded the cardinality restriction, we could ultimately put there the \concept{Element{\_}1} end concept node. But the content rule says, there can be [1..n] elements. We are still bound to insert only one though.
\\
