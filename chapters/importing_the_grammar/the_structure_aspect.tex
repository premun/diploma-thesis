...
As we have stated above, it has several advantages concerning complexity compared to the smart auto-completion and only one disadvantage regarding the cardinality restriction (TODO: ref to cardinality problem).
Even though it is slightly suboptimal, we have analyzed several ANTLR grammars (such as JavaScript\footnote{https://github.com/antlr/grammars-v4/blob/master/ecmascript/ECMAScript.g4}) and we haven't found a single intermediate rule, that would be suffering from this restriction.
...
It would make things very complicated and messy to have it randomly around. And more importantly, that is just not the way languages look like.
\\
